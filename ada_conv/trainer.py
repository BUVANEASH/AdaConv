...
class Trainer:
    ...
    def setup(self):
        ...
        self.content_train_dataloader = InfiniteDataLoader(
            ImageDataset(
                os.path.join(self.hyper_param.data_path, "content/train2017"),
        ...
        self.style_train_dataloader = InfiniteDataLoader(
            ImageDataset(
                os.path.join(self.hyper_param.data_path, "style/train"),
                transform=get_transform(
                    resize=self.hyper_param.resize_size,
                    crop_size=self.hyper_param.image_shape[-1],
                ),
        ...
        self.content_test_dataloader = InfiniteDataLoader(
            ImageDataset(
                os.path.join(self.hyper_param.data_path, "content/test2017"),
                transform=get_transform(
                    resize=self.hyper_param.resize_size,
                    crop_size=self.hyper_param.image_shape[-1],
                ),
        ...
        self.style_test_dataloader = InfiniteDataLoader(
            ImageDataset(
                os.path.join(self.hyper_param.data_path, "style/test"),
                transform=get_transform(
                    resize=self.hyper_param.resize_size,
                    crop_size=self.hyper_param.image_shape[-1],
                ),
